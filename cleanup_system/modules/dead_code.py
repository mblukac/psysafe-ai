...
class DeadCodeAnalyzer(CodeCleanupBase):
    ...
    def find_unused_functions(self) -> List[UnusedFunction]:
        ...
        for func_name, locations in all_functions.items():
            ...
            if (
                ...
                or func_name in ["setUp", "tearDown", "setUpClass", "tearDownClass"]
            ):
                continue
            ...
            if func_name not in all_calls:
                for file_path, line_no in locations:
                    ...
                    if not any(uf.name == func_name and uf.file_path == file_path for uf in unused_functions):
                        unused_functions.append(
                            UnusedFunction(
                                file_path=file_path,
                                name=func_name,
    ...
    def execute(self, dry_run: bool = False) -> CleanupReport:
        ...
        try:
            ...
            report.add_metric("placeholder_functions_removed", sum(1 for f in unused_functions if f.is_placeholder))
    ...
    def _remove_commented_block(self, block: CommentedCodeBlock) -> None:
        ...
        with open(block.file_path, encoding="utf-8") as f:
            lines = f.readlines()
