...
from psysafe.core.models import GuardedRequest, PromptRenderCtx, ValidationReport, ValidationSeverity, Violation
from psysafe.core.prompt import PromptGuardrail
from psysafe.core.template import PromptTemplate
from psysafe.typing.requests import OpenAIChatRequest  # Using OpenAIChatRequest as a concrete example
...
def failing_validator(response: DummyResponse) -> ValidationReport:
    return ValidationReport(
        is_valid=False,
        violations=[Violation(severity=ValidationSeverity.ERROR, code="FAIL", message="Failed")],
        metadata={"validator_name": "failing_validator"},
    )
...
def test_check_guardrail_validate_mixed_validators_with_exception_and_malformed():
    guardrail = CheckGuardrail[DummyRequest, DummyResponse](
        validators=[
            passing_validator,
...
def test_composite_guardrail_apply_multiple_modifying_guardrails():
    ...
    assert guarded_req.metadata["composite_guardrail_sequence"] == [
        "ModifyingPromptGuardrail",
        "ModifyingPromptGuardrail",
        "ModifyingPromptGuardrail",
    ]
    assert "step_0_ModifyingPromptGuardrail" in guarded_req.metadata
...
def test_composite_guardrail_validate_modifying_prompt_guardrail_in_chain():
    ...
    assert (
        final_report.metadata.get("guardrail_type") == "CompositeGuardrail"
    )  # CompositeGuardrail sets its own type after merging children.
