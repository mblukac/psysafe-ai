...
class TestParseLLMResponse:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                '```\n{"no_json_tag": true}\n```',
                {"no_json_tag": True},
            ),  # Test without "json" in markdown, corrected True to true
            (
                '```json\n  {\n    "formatted": true,\n    "indentation": 2\n  }\n```',
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {"analysis": "Test with root", "risk": "3"},
            ),
    ...
    def test_parse_malformed_input(self, raw_response: str):
        ...
        assert any(
            msg in excinfo.value.message for msg in possible_error_messages
    ...
    def test_parse_with_logger(self, caplog):
        ...
        assert (
            "parsing failed" in caplog.text.lower()
            or "all parsing attempts failed" in caplog.text.lower()
            or "xml parsed to an empty dictionary" in caplog.text.lower()
        )
